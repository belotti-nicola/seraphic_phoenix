...
int main()
{
    ...
    uint8_t in[] = TEST;
    exit_code = sp_mpsc_sb_peek(&sb,in,sizeof(TEST));
    if (exit_code == false)
    {
        TEST_ERROR("sp_mpsc_sb_peek failed");
        return 1;
    }

    exit_code = sp_mpsc_sb_advance(&sb,sizeof(TEST));
    if (exit_code == false)
    {
        TEST_ERROR("sp_mpsc_sb_advance failed");
        return 1;
    }
